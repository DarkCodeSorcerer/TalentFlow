...
A full-stack Application Tracking System built with Bun, TypeScript, MongoDB, and React. This system helps recruiters and hiring managers track job applications, manage candidate resumes, and automatically match resumes to job descriptions using intelligent keyword analysis.
...
- This script will:
  - Check if `backend/.env` exists, create it if missing
  - Start the backend server on port 5000
  - Display helpful error messages if MongoDB is not running

**Start Frontend:**
- Double-click `START_FRONTEND.bat` in the project root
- This script will:
  - Check if `frontend/.env` exists, create it if missing
